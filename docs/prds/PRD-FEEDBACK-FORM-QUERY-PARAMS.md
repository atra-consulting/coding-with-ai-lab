...
- Pre-implementation: confirm with the Google Apps Script / spreadsheet owner that `schulung` and `trainerName` columns exist or will be added before this ships.
...
- `submitFeedback()` currently builds `payload` from `timestamp`, `schulungsDatum`, and one entry per question. Add `schulung` and `trainerName` keys sourced from the same resolved values used for display (override-or-default), so the payload and the on-screen text can never disagree.
