...
Three schemes. Some endpoints accept more than one — the write endpoints a skill needs (`create`, `/:id/owner`, `/:id/comments`) and `GET /:id` accept **agent token · loopback bypass · admin session** (first match wins).

### Agent token (machine endpoints: `/next`, `/:id/start`, `/:id/done`, `/:id/ask`, plus `POST /`, `PATCH /:id/owner`, `POST /:id/comments`)
...
**Admin-only** (session required, no token or loopback accepted): `GET /`, `GET /board`, `GET /summary`, `PATCH /:id/status`, `POST /:id/wont-do`, `POST /:id/hand-to-ai`, `POST /reset`.

**Admin session _or_ agent token _or_ loopback bypass** (first match wins): `GET /:id`, `POST /` (create), `PATCH /:id/owner`, `POST /:id/comments`. A wrong token is rejected outright (never falls through to the session check). These let a headless skill file and refine a ticket without an admin login.
...
A workshop skill drives this API as an agent. It uses the **agent-token endpoints** — the claim/finish/ask verbs **plus** three write endpoints a skill needs to file and refine a ticket: `POST /` (create), `PATCH /:id/owner`, and `POST /:id/comments`. The remaining admin endpoints (`board`, `summary`, `list`, `status`, `wont-do`, `hand-to-ai`, `reset`) stay session-only for the human dashboard.
