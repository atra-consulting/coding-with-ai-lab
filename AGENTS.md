...
- Resolution: `done` sets `solution=DONE`; an admin can set `solution=WONT_DO` (`POST /:id/wont-do`, only on `owner=HUMAN` tickets). Most agent endpoints use `requireAgentTokenOrAdminSession` (agent token, loopback bypass, or admin session); only `GET /next` stays agent-token-only. Admin-only endpoints use `requireAuth` + `requireRole('ADMIN')`. The three read endpoints (`GET /board`, `GET /summary`, `GET /:id`) accept any logged-in session, so participants can follow the board. Drag-and-drop board at `/admin/tickets` — read-only for non-admins. Seeded with the 12 workshop specs; `POST /api/tickets/reset` re-seeds.
