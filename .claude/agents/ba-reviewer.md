...
- Database: @libsql/client 0.17 with Drizzle ORM 0.41 (file-based SQLite at `backend/data/crmdb.sqlite`)
- Auth: session-based (`express-session` + DB-backed store), hardcoded users in `backend/src/config/users.ts`, role and permission middleware in `backend/src/middleware/auth.ts`
