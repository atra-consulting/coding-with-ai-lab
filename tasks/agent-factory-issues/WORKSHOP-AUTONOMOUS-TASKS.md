...
curl -s -H "Authorization: Bearer $TOKEN" "$BASE/api/agent-aufgaben/next?source=EMAIL" | jq .

# Reject a task (comment is mandatory; missing comment => 400):
curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"comment":"Too vague to implement"}' "$BASE/api/agent-aufgaben/<id>/reject" | jq .

# Mark a task done:
curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"comment":"Implemented in PR #123"}' "$BASE/api/agent-aufgaben/<id>/done" | jq .
```
...
- **API equivalent:**
  ...
  ```bash
  curl -s -X POST -H "Authorization: Bearer $TOKEN" "$BASE/api/agent-aufgaben/reset" | jq .
  # => { "reset": 16 }
  ```
